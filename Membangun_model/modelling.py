...
# TRAIN + LOG MLflow
# =========================
def latih_dan_log(model, nama_model,
    ...
    with mlflow.start_run(run_name=nama_model):
        ...
        mlflow.log_artifact(
            simpan_confusion_matrix(y_test, y_pred_test, nama_model)
        )
        mlflow.log_artifact(
            simpan_roc_curve(model, X_test, y_test, nama_model)
        )

        laporan = classification_report(
            y_test,
            y_pred_test,
            target_names=["Buruk", "Bagus"]
        )

        report_file = f"report_{nama_model}.txt"
        ...
        return metrik["test_f1"], mlflow.active_run().info.run_id, model
...
if __name__ == "__main__":
    ...
    # 🔥 FIX UTAMA: indentasi loop harus di sini
